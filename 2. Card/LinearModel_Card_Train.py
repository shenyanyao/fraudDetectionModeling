...
for i in range(1,11):
    ...
    current_card=pd.read_csv(current_csv,header=None,nrows = None)
    ...
    #current_card = scaler.fit_transform(current_card)
    ...
    y_pred = regressor.predict(current_card)
    #y_pred = regressor.predict_proba(current_card)[:,1]
    fpr, tpr, thresholds = metrics.roc_curve(y_test, y_pred)
    auc = metrics.auc(fpr, tpr)
    print auc
    
    path='D:/CapitalOne/2. Card/card/linear_nn'+str(i)+'.model'
    s.savemod(path)
